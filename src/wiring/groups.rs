...
pub(crate) fn wire_groups(ui: &WhiteNoiseLinux, cx: &Cx) {
    ...
    wire!(ui, on_demote_admin [backend_cell, group_ids], |ui, member_id| {
        let member_id = member_id.trim().to_string();
        if member_id.is_empty() {
            return;
        }
    ...
    wire!(ui, on_self_demote_admin [backend_cell, group_ids], |ui| {
        let Some((group_hex, b)) = active_group_backend(&ui, &backend_cell, &group_ids) else {
            return;
        };
        spawn_group_admin_op(
    ...
    wire!(ui, on_transfer_ownership [backend_cell, group_ids], |ui, member_id| {
        let member_id = member_id.trim().to_string();
        if member_id.is_empty() {
            return;
        }
    ...
    wire!(ui, on_remove_member [backend_cell, group_ids], |ui, member_id| {
        let member_id = member_id.trim().to_string();
        if member_id.is_empty() {
            return;
        }
    ...
    wire!(ui, on_clear_group_image [backend_cell, group_ids], |ui| {
    ...
    wire!(ui, on_change_group_image [backend_cell, group_ids], |ui| {
        if ui.get_group_image_busy() {
            return;
        }
        let idx = ui.get_active_chat() as usize;
