...
profile_theme_and_shapes :: proc(t: ^testing.T) {
	...
	for bad in ([]string{"{}", `{"kind":0,"tags":[]}`, `{"kind":16767,"content":"{\"background\":\"red\"}"}`,
		`{"kind":16767,"tags":[["c","#123456","background"],["c","#ffffff","text"],["c","#nope00","primary"]]}`,
		`{"kind":16767,"tags":[["c","#123456","background"],["c","#ffffff","text"],["c","#111111","primary"],["c","#222222","primary"]]}`}) {
		testing.expect(t, profile_theme_parse(bad).colors[0].a == 0, "incomplete or malformed themes must leave the app palette intact")
	}
	...
	prefs := Prefs{avatar_shape = .Square, crop_avatar_shape = .Rounded}
	...
	testing.expect(t, round_trip.avatar_shape == .Square && round_trip.crop_avatar_shape == .Rounded)
}
...
profile_theme_layout :: proc(t: ^testing.T) {
	...
	for asset, i in ([2]string{background, font}) {
		sum := string(hex.encode(hash.hash_string(.SHA256, asset, context.temp_allocator), context.temp_allocator))
		path := i == 0 ? fmt.tprintf("%s/events/img/%s", data_home, sum) : fmt.tprintf("%s/events/fonts/%s.ttf", data_home, sum)
		bytes, err := os.read_entire_file(i == 0 ? "assets/whitenoise-linux.png" : "vendor/fonts/LiberationMono-Regular.ttf", context.temp_allocator)
	...
	for kind in ([2]u32{16767, 0}) {
		...
		raw := fmt.tprintf(`{{"kind":16767,"tags":[["c","#171229","background"],["c","#fff2d9","text"],["c","#ffb66d","primary"],["f","Mono","%s","body"],["f","Mono","%s","title"],["bg","url %s","mode tile","m image/png"]]}}`, font, font, background)
		nev_cards[ref] = {done = true, raw = kind == 16767 ? strings.clone(raw) : "", content = kind == 0 ? `{"shape":"⭐"}` : ""}
	}
	...
	ui.profile = {name = "Theme visitor", about = "A profile with Ditto colors, a tiled background and custom fonts.", npub = hex_npub(key)}
	ui.my_pic_url = "test://avatar"
	append(&ui.contacts, Contact_Ui{id_hex = key, name = "Theme visitor", npub = ui.profile.npub, pic_url = ui.my_pic_url})
	...
	testing.expect(t, style.shape == "⭐" && !style.done[0], "avatar lookup must not load profile themes")
	...
	for command in avatar_commands.internalArray[:avatar_commands.length] {
		...
		testing.expect(t, command.renderData.image.imageData == rawptr(shaped_avatar(photo, "⭐")), "published shapes override the local default outside profiles too")
	}
	profile_style(key)
	profile_font(font)
	nev_img(background)
	...
	for pane in 0 ..< 3 {
		for frame in 0 ..< 3 {
			...
			clay.BeginLayout()
			if clay.UI(clay.ID("ProfileTestRoot"))({layout = {sizing = {clay.SizingFixed(1100), clay.SizingFixed(950)}}, backgroundColor = CARD}) {
				if clay.UI(clay.ID("UnstyledSidebar"))({layout = {sizing = {clay.SizingFixed(220), clay.SizingGrow()}, padding = clay.PaddingAll(20)}, backgroundColor = CARD}) {
					avatar("SidebarShape", 0, key, "Visitor", 30, photo)
					clay.Text("App stays unchanged", {fontId = FONT_BODY, fontSize = 14, textColor = TEXT})
				}
				switch pane {
				case 0: contacts_pane(&ui)
				case 1: profile_pane(&ui)
				case 2:
					if clay.UI(clay.ID("SettingsTest"))({layout = {sizing = {clay.SizingGrow(), clay.SizingGrow()}, layoutDirection = .TopToBottom, padding = clay.PaddingAll(20), childGap = 10}}) { settings_appearance(&ui) }
				}
			}
			commands := clay.EndLayout(0)
			testing.expect(t, BG == before, "profile colors must not leak to the sidebar or settings")
			if pane < 2 {
				found_background, found_font := false, false
				for cmd in commands.internalArray[:commands.length] {
					if cmd.commandType == .Custom && (^Model_Kind)(cmd.renderData.custom.customData)^ == .Profile_Background { found_background = true }
					if cmd.commandType == .Text && cmd.renderData.text.fontId >= 32 { found_font = true }
				}
