...
hidden_message_decode :: proc(t: ^testing.T) {
	...
	for text in ([]string{"", "🦂", "👩🏽‍💻", "می\u200cروم", "a\u200bb", a[:len(a) - 3], fmt.tprintf("%s\u200c", a), fmt.tprintf("%s\u200b", a), fmt.tprintf("\u200b%s", a), hidden_fixture("0000000000000000"), hidden_fixture("1101100000111101"), hidden_fixture("1101111000001000"), hidden_fixture("1101100000111101 0000000001000001"), hidden_fixture("0000000001000001 1101111000001000"), hidden_fixture("0000000001000001 000000000100000")}) {
...
hidden_message_layout :: proc(t: ^testing.T) {
	...
	nested := fmt.tprintf(
		"# Secret\n\n**Bold** and *italic*.\n\n🗝️%s",
		secret_fixture("- Nested **markdown**"),
	)
	...
	append(
		&ui.messages,
		Msg_Ui {
			sender = strings.clone("Neighbor"),
			mine = true,
			blocks = parse_md_text("Neighbor 😀"),
		},
	)
	defer {
	...
	for width in ([]i32{600, 320}) {
		...
		for state in 0 ..< 5 {
			for frame in 0 ..< 3 {
				...
				testing.expect(
					t,
					neighbor.y >= clay.GetElementData(clay.ID("MsgRow", 1)).boundingBox.y,
				)
				data := clay.GetElementData(clay.ID("MsgReveal", 0))
				testing.expect(
					t,
					data.found && data.boundingBox.width == 44 && data.boundingBox.height == 44,
				)
				testing.expect_value(
					t,
					clay.GetElementData(clay.ID("MsgReveal", 1)).found,
					state > 0 && state < 4,
				)
				rl.BeginDrawing()
				...
				rl.TakeScreenshot(
					strings.clone_to_cstring(
						fmt.tprintf("/tmp/wn-hidden-%d-%d.png", width, state),
				...
				if state < 4 {
					if state == 1 ||
					   state == 2 {data = clay.GetElementData(clay.ID("MsgReveal", 1))}
