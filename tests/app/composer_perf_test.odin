...
// SDL_VIDEODRIVER=dummy tests/odin.sh app -o:minimal -define:ODIN_TEST_NAMES=composer_performance
@(test)
composer_performance :: proc(t: ^testing.T) {
	...
	for count in ([]int{1500, 6000}) {
		...
		for typing in 0 ..< 2 {
			...
			slice.sort(layout[:])
			fmt.printf(
				"composer bytes=%d typing=%d median_ms=%.3f p95_ms=%.3f layout_ms=%.3f\n",
				len(ui.compose),
				typing,
				samples[7],
				samples[14],
