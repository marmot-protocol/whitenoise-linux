...
hidden_message_perf :: proc(t: ^testing.T) {
	...
	defer {delete(ui.accounts); delete(ui.chats); delete(ui.messages); wrap_clear()
		g_ui, g_prefs, g_client = nil, nil, nil}
	...
	for body, kind in cases {
		last := marmot.Chat_List_Message_Preview {
			plaintext = strings.clone_to_cstring(body, context.temp_allocator),
			sender    = "peer",
			kind      = 9,
		}
		row := marmot.Presented_Chat_Row {
			row = {group_id_hex = "test", last_message = &last},
		}
		row.presentation.title = {
			tag = .Literal,
			body = {literal = "Hidden payload"},
		}
		start := time.tick_now()
		chat := row_to_ui(client, &row, "self")
		cover, secret := hidden_message(body)
		msg := Msg_Ui {
			id      = strings.clone("secret"),
			body    = strings.clone(body),
			sender  = strings.clone("Peer"),
			blocks  = parse_md_text(cover),
			secrets = secret_layers(client, secret),
		}
		fmt.printf(
			"hidden kind=%d bytes=%d load_ms=%.3f\n",
			kind,
			len(body),
		...
		for opened in 0 ..< 2 {
			...
			slice.sort(samples[:])
			fmt.printf(
				"hidden kind=%d open=%d median_ms=%.3f max_ms=%.3f\n",
				kind,
				opened,
				samples[3],
				samples[6],
