...
system_text_participants :: proc(t: ^testing.T) {
	cases := [?]struct {
		...
		{"group_avatar_changed", "Alice", nil, nil, "Alice changed the group photo"},
		{
			...
			nil,
			nil,
			"Alice changed the disappearing message timer",
		},
	...
	for tc in cases {
		ev := marmot.Group_System_Event {
			system_type          = tc.kind,
			text                 = "Fallback",
			actor_display_name   = tc.actor,
			subject_display_name = tc.subject,
			name                 = tc.name,
		}
...
system_mentions_layout :: proc(t: ^testing.T) {
	...
	{
		layout = {
			sizing = {width = clay.SizingFixed(800), height = clay.SizingFixed(320)},
		...
		for kind, i in ([]cstring{"admin_added", "admin_removed", "member_removed", "group_renamed", "member_added"}) {
			ev := marmot.Group_System_Event {
				system_type            = kind,
				subject_account_id_hex = subject,
			...
			msg := Msg_Ui {
				system   = true,
				sys_text = system_text(nil, &ev, .Mentions),
				at       = "08:28",
			}
	...
	for i in 0 ..< 5 {
		...
		testing.expect(
			t,
			chip.found && chip.boundingBox.width > 0,
			"System participant must render as a mention chip",
		)
	}
	rl.BeginDrawing()
