...
send_waits_for_timeline :: proc(t: ^testing.T) {
	...
	records := [?]marmot.Timeline_Message_Record{{message_id_hex = "second-id"}, {message_id_hex = "photo-id"}}
	page := marmot.Timeline_Page{messages = raw_data(records[:]), messages_len = 1}
	timeline_page = &page
	drain_sends(&ui, nil)
	testing.expect_value(t, len(ui.pending), 1)
