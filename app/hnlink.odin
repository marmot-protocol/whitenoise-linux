...
@(private)
Hn_Card :: struct { title, author: string }
...
hn_mutex: sync.Mutex
@(private = "file")
hn_fresh: [dynamic]struct { key: string, card: Hn_Card }
...
hn_worker :: proc(key: string) {
	...
	sync.lock(&hn_mutex)
	append(&hn_fresh, struct { key: string, card: Hn_Card }{key, card})
...
hn_card :: proc(id: u32, key, url: string) {
	...
	if clay.UI(clay.ID("HnCard", id))({
		...
		backgroundColor = PLATE, cornerRadius = rr(12),
		border = {color = hovered() ? fade(ACCENT, 0.5) : CARD_BORDER, width = bw()},
	}) {
		if hovered() { link_hover = url }
		clay.Text("HACKER NEWS", {fontId = FONT_BODY, fontSize = 9, textColor = TEXT_DIM, letterSpacing = 1})
		title := card.title != "" ? card.title : fmt.tprintf("Hacker News #%s", key)
		clay.Text(title, {fontId = FONT_TITLE, fontSize = 15, textColor = TEXT})
		if card.author != "" {
			clay.Text(fmt.tprintf("@%s", card.author), {fontId = FONT_BODY, fontSize = 11, textColor = TEXT_DIM})
		}
