...
stt_draft_safety :: proc(t: ^testing.T) {
	...
	for scenario in 0 ..< 8 {
		...
		case 6: ui.account_ref = "other"
		case 7:
			ui.stt.purpose = .Download
			ui.page = .Settings
			ui.selected = -1
		case 4, 5: append(&ui.compose, " changed")
		}
		...
		want := scenario == 0 ? "Draft こんにちは" : (scenario == 4 || scenario == 5) ? "Draft changed" : "Draft"
...
stt_model_selection :: proc(t: ^testing.T) {
	...
	for model, i in STT_MODELS {
		...
		testing.expect_value(t, model.sizes[0] + model.sizes[1] + model.sizes[2] + model.sizes[3], model.bytes)
...
stt_partial_results :: proc(t: ^testing.T) {
	...
	for attachment in 0 ..< 2 {
		...
		testing.expect(t, file != nil && err == nil)
		ui.stt = {file = file, child = child, group = strings.clone("chat"), draft = strings.clone("Draft")}
		view: Video_View
		if attachment == 1 {
			ui.stt.message = strings.clone("audio")
			msg := Msg_Ui{id = "audio"}
		...
		testing.expect_value(t, string(ui.compose[:]), attachment == 0 ? "Draft Hello world" : "Draft")
...
stt_cache_round_trip :: proc(t: ^testing.T) {
	...
	view := Video_View{transcript = strings.clone("Completed text"), transcript_done = true}
