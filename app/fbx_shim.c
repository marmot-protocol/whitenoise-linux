...
static void dominant_weight(const ufbx_skin_deformer *skin, int32_t bone_base, size_t vertex,
                            int32_t *out_bone, float *out_weight) {
    *out_bone = -1;
    *out_weight = 0.0f;
...
static void subdivide_big_tris(fbx_scene *s) {
    ...
    if (!nm.pos || !nm.nrm || !nm.uv || !nm.bone || !nm.weight || !nm.mat || !ns.geo ||
        !ns.geo_nrm || !ns.vidx || !ns.tripart) {
        free(nm.pos);
        free(nm.nrm);
        free(nm.uv);
        free(nm.bone);
    ...
    m->mat = nm.mat;
    s->geo = ns.geo;
    s->geo_nrm = ns.geo_nrm;
    s->vidx = ns.vidx;
    s->tripart = ns.tripart;
...
fbx_scene *fbx_open(const void *data, size_t len) {
    ...
    if (!m->pos || !m->nrm || !m->uv || !m->bone || !m->weight || !m->mat || !m->mats || !s->geo ||
        !s->geo_nrm || !s->vidx || !s->tripart || !s->world || !s->world_done || !s->skin_mat) {
        fbx_free_arrays(s);
        ufbx_free_scene(scene);
        free(s);
        return NULL;
    ...
    for (int32_t pi = 0; pi < s->num_parts; pi++) {
        ...
        for (size_t f = 0; f < mesh->faces.count; f++) {
            ...
            uint32_t tris =
                ufbx_triangulate_face(corners, mesh->max_face_triangles * 3, mesh, face);
            int32_t material = -1;
            ...
            for (uint32_t t = 0; t < tris; t++) {
                ...
                for (int k = 0; k < 3; k++) {
                    ...
                    dominant_weight(part->skin, part->bone_base, vertex, &m->bone[corner],
                                    &m->weight[corner]);
                    if (part->skin) {
                        m->has_skin = 1;
                    }
...
static int32_t eval_raw(fbx_scene *s, int32_t anim_index, double time, float *out_pos,
    ...
    for (int32_t tri = 0; tri < s->model.num_tris; tri++) {
        ...
        for (int k = 0; k < 3; k++) {
            ...
            if (skin && vertex < skin->vertices.count &&
                skin->vertices.data[vertex].num_weights > 0) {
                ufbx_skin_vertex sv = skin->vertices.data[vertex];
                ufbx_matrix acc = {0};
                double total = 0.0;
                for (uint32_t w = 0; w < sv.num_weights; w++) {
                    ...
                    const ufbx_matrix *cm =
                        &s->skin_mat[part->bone_base + (int32_t)sw.cluster_index];
    ...
    if (s->scene->anim_stacks.count > 0 && m->num_tris <= FBX_FIT_MAX_TRIS) {
        ...
        if (scratch_pos && scratch_nrm) {
            ...
            for (int sample = 0; sample < FBX_FIT_SAMPLES; sample++) {
                double t = stack->time_begin + (stack->time_end - stack->time_begin) *
                                                   ((double)sample / (FBX_FIT_SAMPLES - 1));
