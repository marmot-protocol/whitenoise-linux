...
when #config(WN_RELOAD, false) {
	@(private = "file")
	Dev_Reload_Gate :: enum c.int {
		Busy,
		Ready,
	}
	@(private = "file")
	Dev_Reload_Action :: enum c.int {
		Keep_Running,
		Reload_App,
		Stop_App,
	}
	...
	wn_app_run :: proc "c" (argc: c.int, argv: [^]cstring, generation: c.int) -> c.int {
		...
		runtime.default_temp_allocator_destroy(
			(^runtime.Default_Temp_Allocator)(context.temp_allocator.data),
		)
	...
	when #config(WN_RELOAD, false) {
		...
		if ui != nil {
			if len(ui.staged) > 0 ||
			   ui.editing != "" ||
			   reload_jobs_busy() ||
			   gimg_writes_pending() {gate = .Busy}
			if len(ui.issue_subject) + len(ui.issue_body) + len(ui.issue_labels) > 0 ||
			   ui.issue_ticket != 0 {gate = .Busy}
