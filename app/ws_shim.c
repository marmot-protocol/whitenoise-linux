...
static int send_text(CURL *c, const char *msg, long deadline) {
	...
	unsigned char mask[4] = {(unsigned char)rand(), (unsigned char)rand(), (unsigned char)rand(), (unsigned char)rand()};
...
static void ws_key(char out[25]) {
	...
	for (int i = 0, o = 0; i < 16; i += 3, o += 4) {
		uint32_t v = (uint32_t)raw[i] << 16 | (uint32_t)(i + 1 < 16 ? raw[i + 1] : 0) << 8 | (i + 2 < 16 ? raw[i + 2] : 0);
		out[o] = B64[v >> 18 & 63];
		out[o + 1] = B64[v >> 12 & 63];
...
int wn_ws_fetch(const char *url, const char *req, char *out, size_t cap, long timeout_ms) {
	...
	int hl = snprintf(hs, sizeof hs,
		...
		path, host, key);
	if (hl <= 0 || (size_t)hl >= sizeof hs || send_all(c, (unsigned char *)hs, (size_t)hl, deadline) < 0) {
		goto done;
	}
