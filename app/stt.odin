...
STT_MODELS := [?]struct {
...
} {
	{
		...
		103609903,
		{12937772, 89855401, 816730, 0},
		{"tiny-encoder.int8.onnx", "tiny-decoder.int8.onnx", "tiny-tokens.txt", ""},
		3,
		N_("Multilingual. Detects your language automatically."),
	},
	{
		"base",
		"Whisper Base",
	...
	},
	{
		...
		4,
		N_("25 European languages, including English and Italian."),
	},
	{
		"sensevoice",
		"SenseVoice Small",
		...
		239549735,
		{239233841, 315894, 0, 0},
		{"model.int8.onnx", "tokens.txt", "", ""},
		2,
		N_("Chinese, English, Japanese, Korean and Cantonese."),
	},
}
...
stt_cache_load :: proc(view: ^Video_View, model: int) {
	...
	if !ok ||
	   len(plain) > 16384 ||
	   !utf8.valid_string(string(plain)) ||
	   strings.contains(string(plain), "\x00") {return}
...
stt_start :: proc(
	...
	message: string = "",
	attachment: int = 0,
	purpose: Stt_Purpose = .Transcribe,
) {
	if !ui.prefs.stt_enabled ||
	   ui.stt.file != nil ||
	   (purpose == .Transcribe &&
			   (voice.stream != nil || ui.selected < 0 || ui.selected >= len(ui.chats))) {
		return
	}
	audio: []u8
	view: ^Video_View
	...
	ui.stt = {
		...
		purpose        = purpose,
		ready          = ui.stt.ready,
		selected_model = stt_model(ui.prefs.stt_model),
		account        = strings.clone(ui.account_ref),
		group          = purpose == .Download ? "" : strings.clone(ui.chats[ui.selected].group_id),
		thread         = strings.clone(thread_cur(ui)),
		editing        = strings.clone(ui.editing),
...
stt_tick :: proc(ui: ^Ui_State) {
	   ...
	   (ui.stt.checked_at == 0 || rl.GetTime() - ui.stt.checked_at >= 1) {
		for model, i in STT_MODELS {
			...
			for j in 0 ..< model.count {
				info, err := os.stat(
					fmt.tprintf("%s/stt/%s/%s", data_home, model.revision, model.files[j]),
	...
	if !ui.prefs.stt_enabled ||
	   ui.stt.account != ui.account_ref ||
	   (ui.stt.purpose == .Transcribe &&
			   (ui.page != .Chats ||
					   ui.selected < 0 ||
					   ...
					   (ui.stt.message == "" &&
							   (ui.stt.editing != ui.editing ||
									   ui.stt.draft != string(ui.compose[:]))))) {
		stt_stop(ui)
		return
	}
	status: [3]u8
	...
	if !running && (err != nil || state.exit_code != 0) {
		...
		case 4:
			toast(ui, tr("Couldn't transcribe this audio. Choose a recording under 10 minutes."))
		case:
	...
	if previous == 0 &&
	   at > 0 &&
	   !strings.has_suffix(string(ui.compose[:]), " ") &&
	   !strings.has_suffix(string(ui.compose[:]), "\n") {
...
stt_status :: proc(ui: ^Ui_State) -> string {
	...
	case 'D':
		return fmt.tprintf(
			"%s %d/%d: %d%%",
			tr("Downloading speech model..."),
			ui.stt.model + 1,
			STT_MODELS[ui.stt.selected_model].count,
			ui.stt.percent,
		)
	case 'R':
		return tr("Listening... Finish when you're done (30 seconds maximum).")
	case 'C', 'P':
		return tr("Transcribing...")
	case:
		return(
			ui.stt.message == "" ? tr("Preparing dictation...") : tr("Preparing transcription...") \
		)
	}
}
