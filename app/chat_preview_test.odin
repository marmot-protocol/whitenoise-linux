...
chat_selected_presentation :: proc(t: ^testing.T) {
	last := marmot.Chat_List_Message_Preview{plaintext = "Hello", sender = "peer", kind = 9}
	row := marmot.Presented_Chat_Row{
		row = {group_id_hex = "98d2a13e5226783d4", title = "98d2a13e5226783d4",
			conversation_kind = .DIRECT, pending_confirmation = true, last_message = &last},
	}
	row.presentation.title = {tag = .Literal, body = {literal = "Upbeat Wolf"}}
	row.presentation.peer_id = "peer-public-key"
	row.presentation.avatar = {tag = .Remote_Image, body = {remote = {url = "https://example.org/wolf.png"}}}
	...
	row.presentation.avatar = {tag = .Encrypted_Group_Image, body = {encrypted = {image = {image_hash_hex = "image-hash"}}}}
	...
	row.presentation.avatar = {tag = .Placeholder}
	for title in ([]marmot.Presentation_Text{{tag = .Unnamed_Group}, {tag = .Unavailable_Conversation}}) {
		...
		testing.expect_value(t, fallback.title, title.tag == .Unnamed_Group ? tr("Unnamed group") : tr("Unavailable conversation"))
