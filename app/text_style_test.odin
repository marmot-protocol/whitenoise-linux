...
emphasis_fixture :: proc() -> [dynamic]Md_Block_Ui {
	plain := marmot.Markdown_Inline{tag = .TEXT}
	plain.body.text.content = "Normal "
	bold := marmot.Markdown_Inline{tag = .TEXT}
	bold.body.text.content = "bold "
	both := marmot.Markdown_Inline{tag = .TEXT}
	both.body.text.content = "both 🌻"
	nested := marmot.Markdown_Inline{tag = .EMPH}
	nested.body.emph = {&both, 1}
	children := [2]marmot.Markdown_Inline{bold, nested}
	strong := marmot.Markdown_Inline{tag = .STRONG}
	strong.body.strong = {raw_data(children[:]), 2}
	italic := marmot.Markdown_Inline{tag = .TEXT}
	italic.body.text.content = " italic"
	emph := marmot.Markdown_Inline{tag = .EMPH}
	emph.body.emph = {&italic, 1}
	code := marmot.Markdown_Inline{tag = .CODE}
	code.body.code.content = " *literal*"
	inlines := [4]marmot.Markdown_Inline{plain, strong, emph, code}
	block := marmot.Markdown_Block{tag = .PARAGRAPH}
...
// SDL_VIDEODRIVER=dummy odin test app -define:ODIN_TEST_NAMES=rich_text_layout
@(test)
rich_text_layout :: proc(t: ^testing.T) {
	...
	ui: Ui_State
	g_ui = &ui
	defer { g_ui = nil; preview_close(); wrap_clear(); delete(sel_lines); sel_lines = nil; delete(ui.link_url) }
	blocks := emphasis_fixture()
	defer blocks_free(blocks)
	for width in ([]f32{140, 460}) {
		for frame in 0 ..< 3 {
			clear(&sel_lines)
			clay.BeginLayout()
			if clay.UI(clay.ID("StyledTest"))({layout = {layoutDirection = .TopToBottom, sizing = {width = clay.SizingFixed(width), height = clay.SizingGrow()}, childGap = 8}, backgroundColor = CARD}) {
			...
			for cmd in commands.internalArray[:commands.length] {
				if cmd.commandType == .Text {
					...
					for line in sel_lines {
						...
						x := cmd.boundingBox.x + 0.25 * rl.MeasureTextLine(data.fontId, data.fontSize, part[:1], 0).x - box.x
						testing.expect_value(t, line.start + hit_plain(line.text, x, line.size, line.tile_px, line.fonts), start)
					}
				}
	...
	url := fmt.tprintf("https://imgs.example/%s/end.gif?x=1&y=2", strings.repeat("long/path/", 30, context.temp_allocator))
	text := fmt.tprintf("Before %s after", url)
	draw :: proc(text: string) {
		clear(&sel_lines)
		link_hover = ""
		clay.BeginLayout()
		if clay.UI(clay.ID("LinkTest"))({layout = {layoutDirection = .TopToBottom, sizing = {width = clay.SizingFixed(240)}}}) { body_text(99, text, 14, TEXT, true, 232) }
		clay.EndLayout(0)
	}
	draw(text)
