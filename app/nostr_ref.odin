...
nostr_at :: proc(text: string, at: int) -> (end: int, ref: Nostr_Ref) {
	...
	if at > 0 &&
	   (text[at - 1] >= 'a' && text[at - 1] <= 'z' ||
			   text[at - 1] >= 'A' && text[at - 1] <= 'Z' ||
			   text[at - 1] >= '0' && text[at - 1] <= '9') {return}
	start := at
	...
	if strings.has_prefix(
		text[start:],
		"https://primal.net/e/",
	) {start += len("https://primal.net/e/")}
	prefix := ""
	for p in ([]string{"npub1", "nprofile1", "note1", "nevent1", "naddr1"}) {
		if len(text) - start >= len(p) &&
		   strings.equal_fold(text[start:start + len(p)], p) {prefix = p; break}
	}
	...
	for i := 0; i < len(data); {
		...
		case 1:
			relay := string(value)
			if strings.has_prefix(relay, "wss://") ||
			   strings.has_prefix(relay, "ws://") {append(&hints, relay)}
		case 2:
			if size != 32 {return}
...
nev_request :: proc(key: string) -> string {
	...
	if ref.event_kind < 30000 {
		return fmt.tprintf(
			`["REQ","wn",{{"authors":["%s"],"kinds":[%d],"limit":1}}]`,
			ref.author,
			ref.event_kind,
		)
	}
	...
	if err != nil {return ""}
	return fmt.tprintf(
		`["REQ","wn",{{"authors":["%s"],"kinds":[%d],"#d":[%s],"limit":1}}]`,
		ref.author,
		ref.event_kind,
		string(identifier),
	)
...
nev_relay_urls :: proc(body: []u8, author: string) -> []string {
	...
	for tag in tags {
		...
		if !strings.has_prefix(string(url), "wss://") &&
		   !strings.has_prefix(string(url), "ws://") {continue}
