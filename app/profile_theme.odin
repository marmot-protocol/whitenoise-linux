...
profile_style :: proc(author: string, part := Profile_Style_Part.All) -> Profile_Style {
	...
	for ref, i in style.refs {
		...
		if i == 0 {
			...
			style.colors, style.background, style.tile, style.fonts =
				theme.colors, theme.background, theme.tile, theme.fonts
		}
...
profile_theme_parse :: proc(raw: string) -> (theme: Profile_Style) {
	...
	for tag in tags {
		...
		if name == "f" && len(parts) >= 3 {
			...
			if theme.fonts[index] == "" &&
			   profile_asset_url(string(url)) {theme.fonts[index] = strings.clone(string(url))}
		}
		...
		if profile_asset_url(url) &&
		   (mode == "cover" || mode == "tile") &&
...
@(private)
profile_palette :: proc(
	colors: [len(PROFILE_COLOR_SLOTS)]clay.Color,
) -> (
	old: [len(PROFILE_COLOR_SLOTS)]clay.Color,
) {
...
profile_asset_url :: proc(url: string) -> bool {
	return(
		len(url) <= 4096 &&
		(strings.has_prefix(url, "https://") || strings.has_prefix(url, "http://")) &&
		!strings.contains_any(url, "\r\n\x00") \
	)
}
