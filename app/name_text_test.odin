...
// odin test app -define:ODIN_TEST_NAMES=contact_name_text
@(test)
contact_name_text :: proc(t: ^testing.T) {
	...
	samples := [?]struct {font, text: string}{
		{"NotoSansMath-Regular.ttf", "𝓓𝓮𝓮 𝕂𝕒𝕪 𝔦𝔰𝔬𝔩𝔞𝔟𝔢𝔩𝔩𝔞𝔯𝔱"},
	...
	testing.expect_value(t, rl.MeasureTextLine(FONT_BODY, 24, "A\uFE0E", 0), rl.MeasureTextLine(FONT_BODY, 24, "A", 0))
	rl.SetPixelScale(1)
	rl.BeginDrawing()
	...
	defer delete(memory)
	ui := Ui_State{page = .Contacts, selected_contact = 0, row_menu = -1, member_menu = -1}
	ui.prefs.rail_w = RAIL_W_MIN
	g_ui, g_prefs = &ui, &ui.prefs
	defer { g_ui, g_prefs = nil, nil }
	append(&ui.accounts, "Test")
	append(&ui.contacts, Contact_Ui{id_hex = "contact", name = "🌻 isolabellart", npub = "npub17nd4y1234567890f6950x"})
	for _ in 0 ..< 6 {
	...
	for width in ([]f32{360, 600, 960, 1200}) {
		...
		for id in ([]string{"ContactPage", "ContactHero", "ContactHeroCol", "NickBox", "StartChatBtn", "CopyNpubBtn", "RelaysCard"}) {
			...
			testing.expect(t, element.boundingBox.x + element.boundingBox.width <= width + 0.01, fmt.tprintf("%s at %.0f", id, width))
		}
