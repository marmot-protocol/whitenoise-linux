...
static gboolean on_decide(
	...
	if (uri &&
	    (strncmp(uri, web->origin, strlen(web->origin)) == 0 ||
	     strncmp(uri, "blob:", 5) == 0 ||
...
static void wn_dispatch(struct WnWeb *web, GdkEvent *event, gboolean keyboard) {
	...
	gdk_event_set_device(event, keyboard ? gdk_seat_get_keyboard(seat) : gdk_seat_get_pointer(seat));
...
static void wn_apply(struct WnWeb *web, const struct WnEvent *in) {
	...
	case WN_EV_DOWN:
	case WN_EV_UP: {
		GdkEvent *event = gdk_event_new(in->kind == WN_EV_DOWN ? GDK_BUTTON_PRESS : GDK_BUTTON_RELEASE);
		event->button.x = in->x;
		event->button.y = in->y;
	...
	case WN_EV_KEY_DOWN:
	case WN_EV_KEY_UP: {
		GdkEvent *event = gdk_event_new(in->kind == WN_EV_KEY_DOWN ? GDK_KEY_PRESS : GDK_KEY_RELEASE);
		...
		if (gdk_keymap_get_entries_for_keyval(
			    gdk_keymap_get_for_display(gdk_display_get_default()), in->arg, &keys, &n) &&
		    n > 0) {
...
static void on_console(WebKitUserContentManager *ucm, WebKitJavascriptResult *result, gpointer data) {
	(void)ucm;
	(void)data;
...
static const char *WN_CONSOLE_HOOK =
	...
	"    tag + ': ' + Array.from(args).map(a => (a && a.stack) ? a.stack : String(a)).join(' ')); } catch (e) {} };"
...
int main(int argc, char **argv) {
	...
	web->view = g_object_new(WEBKIT_TYPE_WEB_VIEW, "web-context", context, "user-content-manager", ucm, NULL);
	if (g_getenv("WN_DEBUG")) {
		...
		webkit_user_content_manager_add_script(ucm, webkit_user_script_new(
			WN_CONSOLE_HOOK, WEBKIT_USER_CONTENT_INJECT_ALL_FRAMES,
