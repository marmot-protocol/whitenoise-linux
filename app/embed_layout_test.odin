...
// SDL_VIDEODRIVER=dummy odin test app -define:ODIN_TEST_NAMES=embed_layout
@(test)
embed_layout :: proc(t: ^testing.T) {
	...
	for dim, n in ([][2]int{{8000, 200}, {200, 8000}, {800, 600}}) {
		...
		tex := rl.LoadTextureFromImage({data = raw_data(pixels), width = i32(dim[0]), height = i32(dim[1])})
		delete(pixels)
		defer rl.UnloadTexture(tex)
		msg := Msg_Ui{id = "image-test", sender = "Sender", mine = true}
		...
		for width in ([]f32{280, 800}) {
			for frame in 0 ..< 3 {
				clay.BeginLayout()
				if clay.UI(clay.ID("Timeline"))({layout = {sizing = {width = clay.SizingFixed(width)}, layoutDirection = .TopToBottom}, backgroundColor = CARD}) { message_row(0, msg) }
		...
		for width in ([]i32{900, 360}) {
			...
			for zoom in ([]f32{0, -1, 1, 2}) {
				...
				for frame in 0 ..< 3 {
					clay.BeginLayout()
					if clay.UI(clay.ID("Root"))({layout = {sizing = {width = clay.SizingGrow(), height = clay.SizingGrow()}}, backgroundColor = BG}) { preview_modal(&ui) }
					commands := clay.EndLayout(0)
					if frame < 2 { continue }
					box := clay.GetElementData(clay.ID("PvModal")).boundingBox
					data := clay.GetScrollContainerData(clay.ID("PvScroll"))
					testing.expect(t, box.x >= 0 && box.x + box.width <= f32(width) + 0.1 && box.height <= 900 * PV_MAX_HEIGHT)
					testing.expect(t, clay.GetElementData(clay.ID("PvZoomIn")).found)
					if zoom == -1 { testing.expect(t, data.contentDimensions.width <= data.scrollContainerDimensions.width + 1) }
					if n == 0 && zoom == 0 { testing.expect(t, data.contentDimensions.width > data.scrollContainerDimensions.width) }
					rl.BeginDrawing()
					clay_raylib_render(&commands)
					rl.TakeScreenshot(fmt.ctprintf("/tmp/wn-image-modal-%d-%d-%.0f.png", n, width, zoom))
					rl.EndDrawing()
	...
	for kind in NEV_TEXT_KINDS {
		for count in ([]int{6, 7}) {
			...
			for parsed in 0 ..< 2 {
				card := Nev_Card{kind = kind, done = true, raw = "{}", content = text}
				if parsed == 1 {
				...
				nev_cards[key] = card
				clay.BeginLayout()
				if clay.UI(clay.ID("NostrTest"))({layout = {layoutDirection = .TopToBottom, sizing = {width = clay.SizingFixed(400)}}}) { nev_card(99, key, "note", nil) }
				commands := clay.EndLayout(0)
				testing.expect_value(t, clay.GetElementData(clay.ID("NevMore", 99)).found, count > MESSAGE_LINES)
				for command in commands.internalArray[:commands.length] {
					...
					testing.expect(t, !strings.contains(string(text.chars[:text.length]), "final line"))
				}
