...
CODE_KEYWORDS := []string {
	"as", "assert", "async", "await", "bool", "break", "case", "catch", "char", "class", "const",
	"constexpr", "continue", "def", "default", "defer", "delete", "do", "double", "elif", "else",
	"end", "enum", "except", "extern", "false", "final", "finally", "float", "fn", "for", "foreach",
	"from", "func", "function", "global", "go", "goto", "if", "impl", "import", "in", "int",
	"interface", "is", "lambda", "let", "local", "loop", "match", "mod", "module", "mut", "namespace",
	"new", "nil", "none", "not", "null", "or", "package", "pass", "private", "proc", "protected",
	"public", "raise", "range", "record", "ref", "require", "return", "select", "self", "static",
	"struct", "switch", "template", "then", "this", "throw", "trait", "true", "try", "type",
	"typedef", "union", "unsafe", "use", "using", "var", "void", "when", "where", "while", "with",
	"yield",
}
...
code_lines :: proc(view: ^Code_View, id_seed: u32, limit: int) {
	...
	for line, i in view.lines[:shown] {
		if clay.UI(clay.ID("CodeLine", id_seed + u32(i)))({layout = {sizing = {width = clay.SizingGrow()}}}) {
			clay.Text(
				fmt.tprintf("%4d ", i + 1),
