...
// SDL_VIDEODRIVER=dummy odin test app -define:ODIN_TEST_NAMES=emoji_wrap_layout
@(test)
emoji_wrap_layout :: proc(t: ^testing.T) {
		...
		for width in ([]f32{120, 240, 480, 960}) {
			for text, i in texts {
				for selected in 0 ..< 2 {
					...
					clear(&sel_lines)
					clay.BeginLayout()
					if clay.UI(clay.ID("EmojiBody"))({layout = {layoutDirection = .TopToBottom, sizing = {width = clay.SizingFixed(width)}}}) {
					...
					for line in sel_lines {
						...
						testing.expect(t, box.width <= width + 0.1,
							fmt.tprintf("case %d: rendered width %.1f exceeds %.1f (scale %.3f)", i, box.width, width, scale))
						...
						for cluster, g in utf8.decode_grapheme_iterate(&it) {
							...
							if !emoji { continue }
							tile := clay.GetElementData(clay.ID("SegEmoji", line.id * 128 + u32(segment)))
							testing.expect(t, tile.found)
							for side in 0 ..< 2 {
								b := tile.boundingBox
								offset := line.start + hit_plain(line.text, b.x + b.width * (side == 0 ? 0.25 : 0.75) - box.x, line.size, line.tile_px)
								expected := line.start + g.byte_index + (side == 0 ? 0 : len(cluster))
								testing.expect(t, offset == expected, fmt.tprintf("case %d width %.0f scale %.3f segment %d side %d: offset %d expected %d, tile x %.1f, line %q", i, width, scale, segment, side, offset, expected, b.x - box.x, line.text))
							}
						}
					}
					testing.expect_value(t, sel_lines[len(sel_lines) - 1].start + len(sel_lines[len(sel_lines) - 1].text), len(text))
				}
			}
		}
	}
	...
	ui.sel_on = false
	clay.BeginLayout()
	if clay.UI(clay.ID("EmojiExcerpt"))({layout = {layoutDirection = .TopToBottom, sizing = {width = clay.SizingFixed(480), height = clay.SizingGrow()}, padding = {top = 20}}, backgroundColor = CARD}) {
