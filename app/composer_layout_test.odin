...
composer_layout :: proc(t: ^testing.T) {
	...
	for i in 0 ..< 20 {
		append(&ui.staged, Staged_File{name = fmt.aprintf("attachment-%d-with-a-long-name.wav", i)})
	}
	...
	for zoom in ([]f32{1, 1.875}) {
		...
		for width in ([]i32{1200, 720, 420}) {
			...
			for suffix in ([]string{"tail", "tall", "👩🏽‍💻🔥", "\n\nnext", "", "word word"}) {
				text := fmt.tprintf("%s%s", strings.repeat("a word ", 200, context.temp_allocator), suffix)
				cached := compose_lines(text)
				copy_lines := make([][2]int, len(cached), context.temp_allocator)
				copy(copy_lines, cached)
				testing.expect(t, raw_data(cached) == raw_data(compose_lines(text)), "unchanged draft reuses line breaks")
				wrap_clear()
			...
			for text in ([]string{
				...
				"Hello",
				strings.repeat("the quick brown fox jumps over the lazy dog ", 500, context.temp_allocator),
				strings.repeat("x", 10000, context.temp_allocator),
				strings.repeat("🔥", 100, context.temp_allocator),
				strings.repeat("👩🏽‍💻🇮🇹", 100, context.temp_allocator),
				strings.repeat("1️⃣#️⃣", 100, context.temp_allocator),
				...
				for _ in 0 ..< 8 {
					...
					testing.expect(t, box.x + box.width <= f32(width) + 0.01, "composer fits on every resize frame")
				}
				commands := build_layout(&ui, 0)
				staged := clay.GetScrollContainerData(clay.ID("StagedRow"))
				testing.expect(t, staged.found && staged.contentDimensions.height > staged.scrollContainerDimensions.height)
				...
				for _, i in ui.staged {
					...
					testing.expect(t, box.x + box.width <= f32(width) + 0.01, "attachment removal stays inside the window")
				}
				for i in 1 ..< 3 {
				...
				testing.expect(t, clay.GetElementData(clay.ID("Timeline")).boundingBox.height > 100)
				...
				for id in ([]string{"AttachBtn", "EmojiBtn", "FxBtn", "PollBtn", "DictateBtn", "MicBtn"}) {
					...
					testing.expect(t, button.y >= clip.y + clip.height, "controls sit below the text viewport")
					testing.expect(t, button.x + button.width <= f32(width), "controls fit the pane")
				}
				if clay.GetScrollContainerData(clay.ID("ComposeClip")).contentDimensions.height > clip.height {
					data := clay.GetScrollContainerData(clay.ID("ComposeClip"))
					testing.expect(t, data.scrollPosition.y < 0, "pasting follows the caret to the end")
					...
					for _ in 0 ..< 3 { build_layout(&ui, 0) }
					last_line := clay.GetElementData(clay.ID("ComposeLine", u32(len(compose_lines(text)) - 1))).boundingBox
					testing.expect(t, last_line.y >= clip.y && last_line.y + last_line.height <= clip.y + clip.height + 0.01, "caret remains visible")
				}
				if len(text) > 10 {
					commands = build_layout(&ui, 0)
					rl.BeginDrawing()
				...
				staged.scrollPosition.y = staged.scrollContainerDimensions.height - staged.contentDimensions.height
				build_layout(&ui, 0)
				view := clay.GetElementData(clay.ID("StagedRow")).boundingBox
				last := clay.GetElementData(clay.ID("StagedX", u32(len(ui.staged) - 1))).boundingBox
				testing.expect(t, last.y >= view.y && last.y + last.height <= view.y + view.height + 0.01, "last attachment can be scrolled into view for removal")
			}
		}
	}
}
